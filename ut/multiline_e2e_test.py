# -*- coding: utf-8 -*-
"""多行中文话术端到端测试：调用拆分后的真实模块函数
验证：剪贴板写入 → 粘贴 → 多行撑高 → ElementTree定位send(排除full_screen) → 点击 → 消息真发出"""
import sys, types, time, os, subprocess
...
for mod in ['tkinter', 'tkinter.ttk', 'tkinter.scrolledtext', 'tkinter.messagebox',
            ...
            'winsound', 'pygetwindow']:
    sys.modules[mod] = types.ModuleType(mod)
sys.modules['tkinter'].END = 'end'

from device.ui_locator import (_dump_ui_xml, _find_id_bounds, _find_node_text, _bounds_center,
                        tap_input_box, tap_send_button)
from device.adb_utils import adb_set_phone_clipboard, adb_phone_paste, adb_tap
from core.doubao import DOUBAO_SEND_ID, DOUBAO_FULLSCREEN_ID, DOUBAO_INPUT_ID
...
cleared = (not t) or (t or '').startswith('发消息')
